...
import time
def plot_partitions(node,result,plt):
    ...
    for region in node._nodes:
        plot_partitions(node._nodes[region],result,plt)
def QFA_KDE(input_dir,result_dir,divide_deep,granularity):
    ...
    n = len(x)
    Npt = pow(n, 1 / 2)
    qt = DynamicQuadTree(centerPt=Point(x_center, y_center, 'center'), dimension=dimension, max_points=Npt / granularity,
...
if __name__=="__main__":
    ...
    QFA_KDE(input_dir, result_dir, divide_deep, granularity)
    time_end = time.time()
